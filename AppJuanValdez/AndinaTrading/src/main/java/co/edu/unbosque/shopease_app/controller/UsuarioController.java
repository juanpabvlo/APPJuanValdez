	...
	public ResponseEntity<String> guardarUsuario(@RequestBody UsuarioModel usuario) {
		try {
			...
			contenidoHtml.append(
			    "<html><body style='font-family: Arial, sans-serif; color: #e3180e; background-color: #FAF4F2; padding: 30px;'>");
			contenidoHtml.append(
			...
			contenidoHtml.append(
			    "<img src='https://juanvaldez.com/wp-content/uploads/2022/11/cropped-Website_amigos_v2_logo-1.jpg' alt='Logo Juan Valdez' style='width: 120px; margin-bottom: 20px;'/>");
			contenidoHtml.append("</div>");
			contenidoHtml.append("<h2 style='color:#fa1105; text-align: center;'>¡Bienvenid@ a Amigos Juan Valdez, "
			        + usuario.getNombre() + "!</h2>");
			contenidoHtml.append(
			    "<p style='font-size: 16px; line-height: 1.6;'>Gracias por unirte a nuestra comunidad. Ahora formas parte de una experiencia auténtica en torno al mejor café premium colombiano.</p>");
			contenidoHtml.append(
			    "<p style='font-size: 16px; line-height: 1.6;'>Con tu membresía podrás acumular puntos, disfrutar beneficios exclusivos y recibir noticias y sorpresas directamente en tu correo.</p>");
			contenidoHtml.append("<div style='margin: 30px 0; text-align: center;'>");
			contenidoHtml.append(
			    "<a href='https://www.juanvaldez.com/amigos' style='background-color:#A3082A; color: white; padding: 12px 25px; text-decoration: none; border-radius: 5px;'>Ir a mi cuenta</a>");
			contenidoHtml.append("</div>");
			contenidoHtml.append(
			    "<p style='font-size: 16px;'>¿Tienes preguntas? Escríbenos a <a href='mailto:amigosjv@juanvaldez.com'>servicioalcliente@juanvaldez.com</a>.</p>");
			contenidoHtml.append(
			    "<p style='font-size: 16px;'>Con cariño,<br>El equipo de <strong>Juan Valdez</strong>.</p>");
			contenidoHtml.append("</div>");
			contenidoHtml.append("</body></html>");


			// Enviar el correo con el contenido HTML
			emailService.enviarCorreoHtml(usuario.getEmail(), "¡Bienvenid@ a Amigos Juan Valdez!", contenidoHtml.toString());
